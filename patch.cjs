...
// The replacement text for MemoizedGrid and MemoizedTable
...
code = code.replace("const PRESET_KEYWORDS = SET_PALABRAS_CLAVE_MASTER;", injectedComponents);

// Replace grid block
const gridRegex = /<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">[\s\S]*?<\/div>\s*\) : \(/;
code = code.replace(gridRegex, `<MemoizedGrid filteredLicitaciones={filteredLicitaciones} onSelectLicitacionAI={onSelectLicitacionAI} onAddPostulacion={onAddPostulacion} setAlertModalItem={setAlertModalItem} /> ) : (`);

// Replace table block
const tableRegex = /<div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-xs">[\s\S]*?<\/div>\s*<\/div>\s*\)/;
code = code.replace(tableRegex, `<MemoizedTable filteredLicitaciones={filteredLicitaciones} onSelectLicitacionAI={onSelectLicitacionAI} openAiEvaluator={openAiEvaluator} onAddPostulacion={onAddPostulacion} setAlertModalItem={setAlertModalItem} /> )`);

fs.writeFileSync('src/components/LicitacionesRadarView.tsx', code);
